...
function processRe(elementId, re, flage) {
    switch (flage) {
        // 填充指定列表内容
        case 1:
            fillList(elementId, re);
            break;
            // 将内容展示到指定位置
        case 2:
            JsonsTOTable(elementId, re);
            break;
        case 3:
            console.log("删除成功");
            break;
...
function SelectForSingleFactor0(url0, tableName, filedName, elementId, divId) {
    ...
    $.ajax({
        ...
        cache: false,
        data: {
            "args": tableName + ":" + filedName + ":" + factorName
        },
        dataType: "Json",
        success: function (re) {
            console.log(re);
...
function SelectForSingleFactor1(url0, tableName, filedName, elementId, divId, filedNames) {
    ...
    $.ajax({
        ...
        cache: false,
        data: {
            "args": tableName + ":" + filedName + ":" + factorName + ":" + filedNames
        },
        dataType: "Json",
        success: function (re) {
            console.log(re);
...
function myUpdate(url0, tableName, filedName, value) {
    ...
    $.ajax({
        ...
        async: false,
        data: {
            "args": tableName + ":" + filedName + ":" + value
        },
        success: function (re) {
            re0 = re;
        },
...
function getFormAllDataHaveFile(url0, formId) {
    var re0 = "";
    // var data0 = $("#" + formId).serialize(); //不带文件的表单
    // console.log(data0);
    var data0 = new FormData($("#"+formId)[0]); //带文件的表单数据
    $.ajax({
        ...
        cache: false,
        async: false,
        processData:false, //因为data值是FormData对象，不需要对数据做处理,需设置为false。
        contentType:false, //因为是FormData对象，需设置为false。且已经声明了属性enctype="multipart/form-data"
        data: data0,
        dataType: "Text",
        success: function (re) {
            re0 = re;
