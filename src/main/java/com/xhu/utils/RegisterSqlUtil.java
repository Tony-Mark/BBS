...
public class RegisterSqlUtil extends HttpServlet {
  private RegisterSqlUtil() {}
  private static String savePath;
  private static Properties pros = new Properties();
  static{
    try {
      InputStream in = RegisterSqlUtil.class.getClassLoader().getResourceAsStream("path.properties");
      pros.load(in);
      savePath = pros.getProperty("images.path");
    } catch (IOException e) {
  ...
  public  static void requestToModelHaveFile(String tbName, Object object, HttpServletRequest request)throws ServletException, IOException{
      ...
      if (part.getContentType() == null) {
        ...
        while (parameterNames.hasMoreElements()) {
          ...
          if (request.getParameterValues(String.valueOf(str0)).length > 1) {
            ...
            try {
              field.set(object, String.valueOf(valueString));
        ...
        if (fileName.length() > 2){
          ...
          File file = new File(savePath + "/" + tbName);
          if (!file.exists()){
            file.mkdir();
          }
          part.write(path);
    ...
    if(file0){
      ...
      field.setAccessible(true);
      try {
        field.set(object, String.valueOf(fileNameString));
      } catch (IllegalAccessException e) {
        e.printStackTrace();
      }
    }
  ...
  public static void setCode1(HttpServletRequest request, HttpServletResponse response) {
    try {
      request.setCharacterEncoding("utf-8");
    } catch (UnsupportedEncodingException e) {
      // TODO Auto-generated catch block
